...
- **Условные листы под прайс**: для каждой ISO-метки в сводке суммируется фактический миллиметраж по длинной стороне (из уже разрешённой метки), делится на **номинальную длинную сторону этого формата** из кода (`PricelistFormatEquivalence`), затем применяется округление из `DefaultRounding`. Это **отдельно** от числа страниц PDF. Настройки размеров в `appsettings` не дублируются — при иной трактовке меняются константы в коде или логика.
